...
export default function SongEditForm({ song }: { song: Song }) {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            showToast({ kind: "success", text: "更新しました。" });
            setMessage("更新しました。");
            router.refresh();
        } catch (error) {
            const message = error instanceof Error ? error.message : "更新失敗";
            showToast({ kind: "error", text: `更新失敗: ${message}` });
    ...
    async function handleDelete() {
        ...
        showToast({ kind: "success", text: "曲を削除しました。" });
        router.push("/editor/songs");
        router.refresh();
    }
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4 rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
            ...
            <div className="flex gap-2">
                ...
                <button type="button" onClick={handleDelete} className="rounded-md bg-red-600 px-5 py-2.5 text-sm font-black text-white hover:bg-red-500">
                    削除
                </button>
            </div>
